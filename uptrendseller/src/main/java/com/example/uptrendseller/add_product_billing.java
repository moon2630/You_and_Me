...
public class add_product_billing extends AppCompatActivity {
    ...
    private TextView btnBack, btnSubmit;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_add_product_billing);
        ...
        // Get category and subcategory from intent (for initial display)
        ...
        productRef.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                if (snapshot.exists()) {
                    ...
                    String loadedCategory = snapshot.child("productCategory").getValue(String.class);
                    String loadedSubCategory = snapshot.child("productSubCategory").getValue(String.class);
                    ...
                    Toast.makeText(add_product_billing.this, "Product data not found", Toast.LENGTH_SHORT).show();
                }
                loading.cancel();
            ...
            public void onCancelled(@NonNull DatabaseError error) {
                loading.cancel();
                progressBar.setVisibility(View.GONE);
                Toast.makeText(add_product_billing.this, "Failed to load product data", Toast.LENGTH_SHORT).show();
            }
        });
    }
    ...
    private void submitProduct() {
        ...
        // Update product status to "active" or "published"
        HashMap<String, Object> updates = new HashMap<>();
        updates.put("productStatus", "active");
        updates.put("productLastUpdatedDate", DateHelper.getCurrentDate());

        // Save commission and payout data
        if (sellingPrice > 0) {
            double commissionAmount = CommissionCalculator.calculateCommission(sellingPrice, category, subCategory);
            double sellerEarnings = CommissionCalculator.calculatePayout(sellingPrice, category, subCategory);
                ...
                .addOnCompleteListener(new OnCompleteListener<Void>() {
                    ...
                    public void onComplete(@NonNull Task<Void> task) {
                        ...
                        if (task.isSuccessful()) {
                            StyleableToast.makeText(add_product_billing.this,
                                    "Product submitted successfully!",
                                    R.style.UptrendToast).show();

                            // Navigate to dashboard or product listing
                            new Handler().postDelayed(new Runnable() {
                                @Override
                                public void run() {
                                    Intent intent = new Intent(add_product_billing.this, inventory_product.class);
    ...
    public void onBackPressed() {
        // Navigate back to manufacture details with all data
        Intent intent = new Intent(this, add_product_manufacture_details.class);
        intent.putExtra("Key", productKey);

        // Pass category and subcategory
        if (category != null) intent.putExtra("Category", category);
        if (subCategory != null) intent.putExtra("SubCategory", subCategory);

        // Pass price data
        intent.putExtra("originalPrice", String.valueOf(originalPrice));
        intent.putExtra("sellingPrice", String.valueOf(sellingPrice));
